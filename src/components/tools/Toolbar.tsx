...
export default function Toolbar({
  ...
  const handleMentionReceived = (mention: SessionChatMention) => {
    ...
    if (currentUser) {
      playSoundWithSettings('chatNotificationSound', currentUser.settings, 0.7).catch(
        (error) => {
  ...
  const handleAtisUpdateFromSocket = (data: {
    ...
    if (data.atis?.letter) {
      ...
      if (data.updatedBy !== currentUser?.username || data.isAutoGenerated) {
        setAtisFlash(true);
  ...
  useEffect(() => {
    if (!sessionId || !accessId || !currentUser) return;
  ...
  }, [sessionId, accessId, currentUser]);
    ...
    >
      ...
      >
        <div className="relative flex">
          {activeUsers.slice(0, 5).map((sessionUser, index) => {
            const isCurrentUser = sessionUser.id === currentUser?.userId;
            const planRole =
              isCurrentUser &&
              (effectivePlan === 'basic' || effectivePlan === 'ultimate')
                ? {
                    id: effectivePlan === 'basic' ? -3 : -4,
                    name: effectivePlan === 'basic' ? 'Basic' : 'Ultimate',
                    color: effectivePlan === 'basic' ? '#ef4444' : '#a855f7',
                    icon: effectivePlan === 'basic' ? 'TicketsPlane' : 'BiSolidBalloon',
                    priority: 999997,
                  }
                : null;
            const baseRoles = sessionUser.roles ?? [];
            const displayRoles = planRole
              ? [...baseRoles, planRole]
              : baseRoles;
            const highestRole = getHighestRole(displayRoles);
            ...
            return (
              ...
              >
                <img
                  src={getAvatarUrl(sessionUser.avatar)}
